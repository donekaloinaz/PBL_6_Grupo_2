...
variables = newt.Properties.VariableNames;
%% Data partition
Y = newt{:,end};
X = newt{:,1:end-1};
...
mnr_mdl = fitmnr(X_train(:,varidx), Y_train);
%Model is tested 
[Y_mnr_pred, mnr_scores] = predict(mnr_mdl, X_test(:,varidx));
...
opt.Verbose = 0;
opt.ShowPlots = false;
knn_mdl_opt = fitcknn(X_train(:,varidx),Y_train,'OptimizeHyperparameters','all', ...
    'HyperparameterOptimizationOptions',opt);
[Y_knn_final_pred, knn_final_scores] = predict(knn_mdl_opt, X_test(:,varidx));
...
svm_mdl_opt = fitcsvm(X_train(:,varidx),Y_train,'OptimizeHyperparameters','all', ...
    'HyperparameterOptimizationOptions',opt);
[Y_svm_final_pred, svm_final_scores] = predict(svm_mdl_opt, X_test(:,varidx));
...
%% Decision Trees
tree_mdl_opt = fitctree(X_train(:,varidx),Y_train,'OptimizeHyperparameters','all', ...
    'HyperparameterOptimizationOptions',opt);
[Y_tree_final_pred, tree_final_scores] = predict(tree_mdl_opt, X_test(:,varidx));
...
%% Random Forest
rf_mdl = fitcensemble(X_train(:,varidx),Y_train,'Method','Bag','Learners','tree','NumLearningCycles',300);
[Y_rf_pred, rf_scores] = predict(rf_mdl,X_test(:,varidx));
...
%% Naive Bayes
nb_mdl = fitcnb(X_train(:,varidx),Y_train,'OptimizeHyperparameters','all', ...
    'HyperparameterOptimizationOptions',opt);
[Y_nb_pred, nb_scores] = predict(nb_mdl, X_test(:,varidx));
...
ens_mdl = fitcensemble(X_train(:,varidx),Y_train,'Learners','tree','OptimizeHyperparameters','all', ...
    'HyperparameterOptimizationOptions',opt);
[ens_predict, ens_scores] = predict(ens_mdl,X_test(:,varidx));
